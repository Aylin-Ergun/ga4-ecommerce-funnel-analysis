-- E-Ticaret Hunisi Projesi - Landing Page Sorgusu
...
SELECT
  coalesce(landing_page, '/unknown'),
  COUNT(*) total_sessions,
  sum(has_view_item) AS product_view_sessions,
  sum(has_add_to_cart) AS add_to_cart_sessions,
  sum(has_purchase) AS purchase_sessions,
  round(safe_divide(sum(has_view_item), COUNT(*)) * 100, 2) AS view_item_rate,
  round(safe_divide(sum(has_add_to_cart), COUNT(*)) * 100, 2)
    AS add_to_cart_rate,
  round(safe_divide(sum(has_purchase), COUNT(*)) * 100, 2) AS purchase_rate,
FROM
  session_base
GROUP BY
  1
HAVING
  COUNT(*) > 1000  -- küçük sample temizledik
ORDER BY
  total_sessions DESC;
